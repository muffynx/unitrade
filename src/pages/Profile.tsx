...
const Profile = () => {
    ...
    try {
      ...
      const soldProducts = response.data.map((product: any) => {
        ...
        if (!mappedProduct.sold) {
          console.warn(
            "⚠️ พบสินค้าที่ sold=false แต่ backend ส่งมาใน sold=true:",
            {
              id: mappedProduct.id,
  ...
  const fetchUserListings = async () => {
    try {
      ...
      if (wrongProducts.length > 0) {
        console.warn(
          `⚠️ พบสินค้าที่ sold=true แต่ backend ส่งมาใน sold=false (${wrongProducts.length} รายการ):`
        );
        console.table(
          wrongProducts.map((p: any) => ({
            id: p._id,
            title: p.title,
            sold: p.sold,
          }))
  ...
  const fetchFavoriteListings = async () => {
    try {
      ...
      const favoriteIds =
        favoritesResponse.data.favorites || favoritesResponse.data;
      ...
      const favoriteProducts = await Promise.all(
        favoriteIds.slice(0, 20).map(async (id: string) => {
          // Limit 20 to avoid too many requests
          try {
      ...
      setFavoriteListings(
        favoriteProducts.filter((p): p is Listing => p !== null)
      );
    } catch (err: any) {
      setError(
        err.response?.data?.message ||
          err.message ||
          "ไม่สามารถดึงรายการโปรดได้"
      );
  ...
  const fetchUserMessages = async () => {
    try {
      ...
      setError(
        err.response?.data?.message || err.message || "ไม่สามารถดึงข้อความได้"
      );
  ...
  const handleMarkAsSold = async (listingId: string) => {
    if (
      !window.confirm(
        "คุณแน่ใจหรือไม่ว่าต้องการทำเครื่องหมายว่าสินค้านี้ขายแล้ว?"
      )
    )
      return;
    ...
    try {
      ...
      await Promise.all([fetchSoldListings(), fetchUserListings()]);
    ...
    const fetchUserData = async () => {
      try {
        ...
        const completeUser: User = {
          ...
          createdAt:
            userData.createdAt || userData.joinDate || new Date().toISOString(),
          completedTrades:
            userData.completedTrades ||
            userData.tradesCompleted ||
  ...
  const handleDeleteListing = async (listingId: string) => {
    ...
    try {
      ...
      setUser((prev) =>
        prev
          ? { ...prev, listings: Math.max(0, (prev.listings || 1) - 1) }
          : prev
      );
      alert(
        deletedFromBackend ? "ลบสินค้าสำเร็จ" : "ลบสินค้าสำเร็จ (frontend only)"
      );
    } catch (err: any) {
      const errorMessage =
        err.response?.data?.message || err.message || "ไม่สามารถลบสินค้าได้";
      setError(errorMessage);
  ...
  const handleRemoveFavorite = async (listingId: string) => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message ||
        err.message ||
        "ไม่สามารถลบออกจากรายการโปรดได้";
      setError(errorMessage);
  ...
  const handleDeleteMessage = async (messageId: string) => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message || err.message || "ไม่สามารถลบข้อความได้";
      setError(errorMessage);
      alert("เกิดข้อผิดพลาด: " + errorMessage);
    }
  };
          ...
          <div className="flex-1">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
              ...
              <div className="flex gap-3">
                {/* 🔧 FIX: เปลี่ยนจาก /profile/edit เป็น /settings */}
                <Link
                  to="/settings"
                ...
                {/* 🔧 FIX: เปลี่ยนจาก alert เป็นนำไปหน้า /settings */}
                <Link
                  to="/settings"
            ...
            <p className="text-gray-500 mb-4">
              {user.university} • เข้าร่วม {joinDate}
            </p>
            ...
            <div className="flex flex-wrap gap-6">
              ...
              <div className="flex flex-col items-center">
                ...
                <p className="text-xl font-bold text-gray-900">
                  {userListings.length + soldListings.length}
                </p>
          ...
          {loading ? (
                  ...
                  <Link to={`/product/${listing.id}`} className="block">
                    <div className="relative">
                      <img
                        src={
                          listing.images[0] ||
                          "https://via.placeholder.com/360x192?text=No+Image"
                        }
                        alt={listing.title}
                      ...
                      />
                      <span
                        className={`absolute top-2 left-2 text-white text-xs px-2 py-1 rounded-full font-medium shadow-sm ${
                  ...
                  <div className="p-4">
                    ...
                    <p
                      className={`text-sm line-clamp-2 mb-3 ${
                        listing.sold ? "text-gray-600" : "text-gray-500"
                      }`}
                    >
                      {listing.description}
                    </p>
            ...
            <div className="text-center py-12 bg-gray-50 rounded-xl">
              ...
              <p className="text-gray-500 mb-6">
                เริ่มขายสินค้าของคุณให้เพื่อนนักศึกษา
              </p>
              <button
                onClick={handleAddNewListing}
                className="bg-blue-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-blue-700 transition-colors"
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="text-center py-12 bg-gray-50 rounded-xl">
              <FaBox size={48} className="mx-auto text-gray-300 mb-4" />
              <h3 className="text-lg font-medium text-gray-700 mb-2">
                ยังไม่มีรายการที่ขายแล้ว
              </h3>
              <p className="text-gray-500 mb-6">
                รายการสินค้าที่คุณขายจะแสดงที่นี่
              </p>
            </div>
          )}
        </div>
        ...
        <div>
          ...
          {favoritesLoading ? (
            ...
            <div className="text-center py-12 bg-gray-50 rounded-xl">
              ...
              <p className="text-gray-500 mb-6">
                บันทึกรายการที่คุณสนใจเพื่อดูในภายหลัง
              </p>
              <Link
                to="/browse"
                className="bg-blue-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-blue-700 transition-colors"
        ...
        <div>
          ...
          {messageLoading ? (
                ...
                >
                      ...
                      <div className="flex items-center gap-3 mb-2 flex-wrap">
                        ...
                        <span
                          className={`text-xs px-2 py-1 rounded-full text-white ${
                            msg.category === "electronics"
                              ? "bg-purple-500"
                              : msg.category === "furniture"
                              ? "bg-yellow-500"
                              : msg.category === "textbooks"
                              ? "bg-green-500"
                              : msg.category === "sports"
                              ? "bg-blue-500"
                              : "bg-gray-500"
                  ...
                  {msg.budget && (
                    <p className="text-sm font-medium text-blue-600">
                      งบประมาณ: ฿{msg.budget.toLocaleString()}
                    </p>
                  )}
                </div>
              ))}
            ...
            <div className="text-center py-12 bg-gray-50 rounded-xl">
              ...
              <p className="text-gray-500 mb-6">
                โพสต์ข้อความเพื่อมองหาสินค้าที่ต้องการ
              </p>
              <Link
                to="/browse"
                className="bg-blue-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-blue-700 transition-colors"
