...
const Settings: React.FC = () => {
  ...
  const fetchUserProfile = useCallback(async () => {
    ...
    try {
      ...
      setMessage({
        text: t("failed_to_load_profile") || "Failed to load profile",
        type: "error",
      });
    }
  ...
  const handleImageUpload = async (
    ...
    if (file.size > 5 * 1024 * 1024) {
      // 5MB limit check
      setMessage({
        text: t("file_too_large") || "File size exceeds 5MB limit.",
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      setMessage({
        text:
          t("profile_image_updated") || "Profile image updated successfully",
        type: "success",
      });
    } catch (error: any) {
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!profileForm.name.trim()) {
        setMessage({
          text: t("name_required") || "Name is required",
          type: "error",
        });
        setLoading(false);
        return;
      ...
      if (!profileForm.username.trim()) {
        setMessage({
          text: t("username_required") || "Username is required",
          type: "error",
        });
        setLoading(false);
        return;
      ...
      if (!profileForm.email.trim()) {
        setMessage({
          text: t("email_required") || "Email is required",
          type: "error",
        });
        setLoading(false);
        return;
      ...
      const errorMsg =
        error.response?.data?.message || t("update_failed") || "Update failed";
      setMessage({ text: errorMsg, type: "error" });
    } finally {
      setLoading(false);
    }
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    if (!passwordForm.currentPassword) {
      setMessage({
        text: t("current_password_required") || "Current password is required",
        type: "error",
      });
      return;
    }
  ...
  const handleLanguageChange = (lng: string) => {
    ...
    setMessage({
      text: t("language_changed") || "Language changed",
      type: "success",
    });
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      setMessage({
        text: t("account_deletion_failed") || "Account deletion failed",
        type: "error",
      });
    } finally {
      setLoading(false);
    ...
    return (
      <div className="relative group w-28 h-28 flex-shrink-0">
        ...
        <button
          ...
          disabled={isUploading}
          className={`absolute inset-0 w-full h-full rounded-full flex items-center justify-center bg-black bg-opacity-40 text-white opacity-0 transition-opacity group-hover:opacity-100 ${
                      ...
                      <div className="flex items-center justify-between flex-wrap gap-4">
                        <div>
                          ...
                          <p className="text-sm text-red-600 mt-1">
                            {t("delete_account_confirm_warning") ||
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-gray-600 mt-1">
                            {t("enable_email_notifications_description") ||
                    ...
                    <div className="p-4 border border-gray-200 rounded-lg hover:border-blue-300 transition-colors">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-gray-600 mt-1">
                            {t("enable_push_notifications_description") ||
