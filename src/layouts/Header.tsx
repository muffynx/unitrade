...
const Header = () => {
    ...
    if (token) {
      ...
      const buildNotifications = async () => {
        ...
        try {
          ...
          try {
            const adminNotifResp = await axios.get(
          ...
          // Conversations
          const convResp = await axios.get(`${API_URL}/api/conversations`, {
            headers,
          });
          const conversations = Array.isArray(convResp.data)
            ? convResp.data
            : [];
          ...
          const messageIdToComments: Record<
            string,
            {
              title: string;
              latestAt: string;
          ...
          myMessages.forEach((m: any) => {
            if (Array.isArray(m.comments)) {
              const commentsReadAt = m.commentsReadAt
                ? new Date(m.commentsReadAt).getTime()
                : 0;

              m.comments.forEach((cm: any) => {
                const commentTime = new Date(
                  cm.createdAt || m.updatedAt
                ).getTime();

                if (
                  (!myUserId || cm.user?._id !== myUserId) &&
                  commentTime > commentsReadAt
                ) {
                  ...
                  bucket.count += 1;
                  const ts =
                    cm.createdAt || m.updatedAt || new Date().toISOString();
                  if (
                    new Date(ts).getTime() > new Date(bucket.latestAt).getTime()
                  ) {
                    bucket.latestAt = ts;
          ...
          const filtered = items.filter(
            (n) => new Date(n.time).getTime() > clearedAt
          );
          filtered.sort(
  ...
  const markAllRead = async () => {
    try {
      ...
      setNotificationsCount((prev) =>
        Math.max(0, prev - unreadChatConvs.length)
      );
    } catch (err) {
  ...
  const handleNotificationClick = async (n: NotificationItem) => {
    ...
    if (n.type === "chat" && n.meta?.conversationId) {
      setUnreadChatConvs((prev) =>
        prev.filter((id) => id !== n.meta.conversationId)
      );
    }
  ...
  const renderAvatar = (size: "sm" | "lg", includeName: boolean = false) => {
    ...
    return (
      <div
        className={`relative flex items-center ${includeName ? "gap-2" : ""} ${
          includeName && size === "sm" ? "py-1 px-2" : ""
        }`}
      >
        {avatarUrl ? (
          <img
        ...
        <div
          className={`rounded-full bg-blue-500 items-center justify-center text-white font-bold ${
            avatarUrl ? "hidden" : "flex"
          } ${avatarClasses} flex-shrink-0`}
        >
          {userData
            ? userData.name
        ...
        {includeName && (
          <span className="text-sm font-medium whitespace-nowrap overflow-hidden text-ellipsis max-w-[120px] hidden md:inline">
    ...
    >
        ...
        <div className="flex items-center space-x-3 lg:space-x-4 lg:min-w-max">
            ...
            <div className="hidden lg:flex items-center space-x-3">
              ...
              >
                ...
                {notifications.filter((n) => n.type === "chat").length > 0 && ( // Use live notification count for badge
                  <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-semibold px-1 min-w-[17px] h-[17px] rounded-full flex items-center justify-center">
                    {notifications.filter((n) => n.type === "chat").length > 99
                      ? "99+"
                        ...
                        notifications.slice(0, visibleCount).map((n) => (
                          ...
                          >
                            ...
                            <div className="text-xs text-gray-400 mt-1">
                              {new Date(n.time).toLocaleString(i18n.language)}
                            </div>
                          </button>
                        ))
                      )}
                    ...
                    <div className="flex gap-2 p-3 border-t border-gray-100 bg-gray-50">
                      {visibleCount < notifications.length && (
                        <button
                          onClick={() =>
                            setVisibleCount((v) =>
                              Math.min(v + PAGE_SIZE, notifications.length)
                            )
                          }
          ...
          <div
            ...
            className={`hidden lg:flex gap-2 items-center ${
              isLoggedIn || !isLoggedIn ? "border-l pl-3" : ""
            } border-gray-200`}
          >
            <button
              onClick={() => changeLanguage("en")}
              className={`text-sm py-1 font-${
                i18n.language === "en" ? "semibold" : "normal"
              } text-${
                i18n.language === "en" ? "blue-500" : "gray-600"
            ...
            <button
              ...
              className={`text-sm py-1 font-${
                i18n.language === "th" ? "semibold" : "normal"
              } text-${
                i18n.language === "th" ? "blue-500" : "gray-600"
              } hover:text-blue-500 transition-colors`}
            >
              TH
            </button>
                  ...
                  <div className="absolute right-0 mt-2 bg-white rounded-xl shadow-2xl w-56 p-2 z-50 border border-gray-100">
                    ...
                    <div className="mt-2 pt-2 border-t border-gray-100">
                      <button
                        ...
                        className="flex items-center gap-3 p-2 text-red-500 text-sm w-full text-left rounded-lg hover:bg-red-50 transition-colors"
                      >
            ...
            {isLoggedIn && (
              <>
                ...
                >
                  ...
                  {notifications.filter((n) => n.type === "chat").length >
                    0 && (
                    <span className="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-[10px] font-semibold px-1 min-w-[17px] h-[17px] rounded-full flex items-center justify-center">
                      {notifications.filter((n) => n.type === "chat").length >
                      99
                        ? "99+"
                        : notifications.filter((n) => n.type === "chat").length}
                ...
                <div ref={notifRef} className="relative">
                  ...
                  {isNotifOpen && (
                    ...
                    >
                      {" "}
                      (Notification Body - Omitted for brevity){" "}
                    </div>
                  )}
                </div>
              </>
      ...
      <nav
        className={`w-full lg:hidden ${
          isMobileMenuOpen
            ? "flex flex-col p-2 pt-4 space-y-4 border-t border-gray-100 mt-3"
            : "hidden"
        }`}
      >
        ...
        {!isLoggedIn && (
          <div className="flex gap-3 pt-2">
            <a
              href="/login"
              className="flex-1 text-center py-2 px-4 text-gray-700 text-base font-medium rounded-lg transition-colors border border-gray-300 hover:bg-gray-50"
            ...
            <a
              href="/register"
              className="flex-1 text-center py-2 px-4 bg-blue-500 text-white text-base font-medium rounded-lg transition-colors hover:bg-blue-600"
              onClick={() => setIsMobileMenuOpen(false)}
            >
              {t("sign_up")}
            </a>
        ...
        <div className="flex flex-col space-y-1 border-b border-gray-100 pb-4">
          <h4 className="text-xs font-semibold uppercase text-gray-500 px-3 pb-1 pt-2">
            {t("navigation")}
          </h4>
          <a
            href="/browse"
            className="flex items-center gap-3 p-2 text-gray-700 text-base font-medium rounded-lg hover:bg-gray-100 transition-colors"
            onClick={() => setIsMobileMenuOpen(false)}
          >
          ...
          <a
            ...
            onClick={() => setIsMobileMenuOpen(false)}
          >
            <FaComment size={16} className="text-blue-500" />
            <span>{t("contact")}</span>
          </a>
        </div>
        ...
        {isLoggedIn && (
          <div className="w-full space-y-1">
            <h4 className="text-xs font-semibold uppercase text-gray-500 px-3 pb-1">
              {t("account")}
            </h4>
            <a
              href="/profile"
              className="flex items-center gap-3 p-2 text-gray-700 text-base no-underline rounded-lg hover:bg-gray-100 transition-colors"
              onClick={() => setIsMobileMenuOpen(false)}
            >
            ...
            <a
              ...
              onClick={() => setIsMobileMenuOpen(false)}
            >
              <FaCog size={16} className="text-blue-500" />
            ...
            <div className="mt-2 pt-2 border-t border-gray-100">
              <button
                ...
                className="flex items-center gap-3 p-2 text-red-500 text-base w-full text-left rounded-lg hover:bg-red-50 transition-colors"
              >
